...
pesquisa(largura,Ln,Sol):- pesquisa_largura(Ln,Sol,0,N), write(visitou(N)),nl.

%pesquisa_largura([],_):- !,fail.

pesquisa_largura([no(E,Pai,Op,C,P)|_],no(E,Pai,Op,C,P),N,M):-

               estado_final(E), M is N+1.



pesquisa_largura([E|R],Sol,N,M):- expande(E,Lseg), esc(E),

  insere_fim(Lseg,R,Resto), M1 is N+1,

                           pesquisa_largura(Resto,Sol,M1,M).

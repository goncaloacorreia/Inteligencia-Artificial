...
irmao(X, Y) :- filho(X,P), homem(P, _, _), filho(X,M), filho(Y,P), filho(Y,M), P\=M, X\=Y.

primoDireito(X, Y) :- filho(X, P), filho(Y, P1), irmaos(P, P1).

irmaos(X, XY) :- findall(Y, irmao(X, Y), XY).

esposa(E, H) :- filho(X, E), filho(X, H), E\=H.

primos(X, Y) :- primoDireito(X, Y).
primos(X, Y) :- primoDireito(X, Z), filho(Z, Y). 
